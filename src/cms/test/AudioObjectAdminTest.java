...
public final class AudioObjectAdminTest {
    ...
    public static void run(TestRunner.TestContext ctx) {
        ...
        ctx.test("stored javascript: and data: URLs render as inert text, never as links", () -> {
            String jsId = Helpers.seedWith(ENTITY, Map.of("contentUrl", "javascript:alert(1)"), jar);
            ...
            String dataId = Helpers.seedWith(ENTITY, Map.of("contentUrl", "data:text/html,x"), jar);
        ...
        ctx.test("stored http(s) URL renders as a clickable link", () -> {
            String id = Helpers.seedWith(ENTITY, Map.of("contentUrl", "https://example.com/profile"), jar);
            Helpers.Response r = Helpers.adminGet(BASE + "/" + id, jar);
            Assert.equal(200, r.status);
